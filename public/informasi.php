<?php
   ...
   $informations = getDataFromDatabase($informationsDb, "SELECT * FROM informations");
   ...
   <div class="max-w-3xl mx-auto min-h-screen bg-white overflow-hidden">
      ...
      <main class="p-5">
         ...
         <section class="flex flex-col gap-5">
            <div class="flex flex-col">
               <span class="text-[20px] font-semibold">Informasi</span>
               <span>Semua informasi akan ditampilkan disini</span>
            </div>
            <hr>
            <?php if (!empty($informations)) : ?>
               <div class="flex flex-col gap-5">
                  <?php foreach (array_reverse($informations) as $information) : ?>
                     <div class="pl-5 flex flex-col gap-3 border-l">
                        <div class="w-fit p-3 bg-white border shadow-md shadow-gray-200 rounded">
                           <div class="flex items-center gap-3">
                              <div class="w-[30px] h-[30px] overflow-hidden rounded-full">
                                 <img class="w-full h-auto object-cover flex items-center justify-between" src="/public/assets/images/icon/icon.png" alt="Tokoisicash">
                              </div>
                              <div class="flex flex-col">
                                 <span>Tokoisicash</span>
                                 <span class="text-[13px] text-gray-500"><?= htmlspecialchars($information["time"]); ?></span>
                              </div>
                           </div>
                        </div>
                        <div class="p-5 bg-gradient-to-b from-white to-gray-100 border shadow-md shadow-gray-200 rounded">
                           <div class="flex flex-col gap-3">
                              <span class="font-semibold"><?= htmlspecialchars($information["title"]); ?></span>
                              <span><?= nl2br(htmlspecialchars($information["message"])); ?></span>
                           </div>
                        </div>
                     </div>
                  <?php endforeach; ?>
            ...
            <?php else : ?>
               <div class="h-[300px] p-5 bg-gray-50 rounded">
                  <div class="w-fit mx-auto text-center flex flex-col">
                     <span class="text-[20px] font-semibold">Belum ada informasi</span>
                     <span>Kami akan selalu memberitahu kamu jika ada informasi terbaru disini</span>
                  </div>
               </div>
            <?php endif; ?>
         </section>
