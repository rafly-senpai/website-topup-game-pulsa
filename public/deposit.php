<?php
   ...
   $deposito = getDataFromDatabase($depositoDb, "SELECT * FROM deposito WHERE user_id = '$userId'");
   ...
   <div class="max-w-3xl mx-auto min-h-screen bg-white overflow-hidden">
      ...
      <main class="p-5">
         <div class="flex flex-col gap-5">
            ...
            <form class="bg-white border shadow-md shadow-gray-200 overflow-hidden rounded" action="/app/services/deposito-process.php" method="post">
               ...
               <div class="p-5">
                  <div class="flex flex-col gap-5">
                     <?php if (isset($_SESSION["depositoError"])) : ?>
                        <div class="w-fit">
                           <span class="text-red-500"><?= htmlspecialchars($_SESSION["depositoError"]); ?></span>
                        </div>
                        <?php unset($_SESSION["depositoError"]); ?>
                     <?php endif; ?>
                     <input class="p-3 bg-white border outline-none rounded" type="text" id="amountInput" name="amountInput" placeholder="Masukkan jumlah deposit" required>
            ...
            <section class="flex flex-col gap-5">
               <div class="flex flex-col">
                  <span class="text-[20px] font-semibold">Riwayat Deposit</span>
                  <span>Semua riwayat deposit kamu akan ditampilkan disini</span>
               </div>
               <hr>
               <?php if (!empty($deposito)) : ?>
                  <div class="flex flex-col gap-3">
                     <?php foreach (array_reverse($deposito) as $dps) : ?>
                        <div class="flex flex-col">
                           <div class="w-fit">
                              <span class="text-gray-500"><?= htmlspecialchars($dps["time"]); ?></span>
                           </div>
                           <a class="p-3 bg-white border shadow-md shadow-gray-200 rounded" href="/public/transaksi/detail-deposit.php?id=<?= htmlspecialchars($dps["deposito_id"]); ?>">
                              <div class="flex items-center justify-between gap-3">
                                 <div class="flex items-center gap-3">
                                    ...
                                    <div class="flex flex-col">
                                       <span><?= htmlspecialchars(number_format($dps["amount"], 0, ",", ".")); ?></span>
                                       <span class="text-[13px] text-gray-500">Deposit</span>
                                    </div>
                                 </div>
                                 <div class="text-end flex flex-col">
                                    <span>IDR <?= htmlspecialchars(number_format($dps["amount"] + 0, 0, ",", ".")); ?></span>
                                    <?php if ($dps["status"] === 0) : ?>
                                       <span class="text-[13px] text-orange-500">Pending</span>
                                    <?php elseif ($dps["status"] === 1) : ?>
               ...
               <?php else : ?>
                  <div class="h-[300px] p-5 bg-gray-50 rounded">
                     <div class="w-fit text-center flex flex-col">
                        <span class="text-[20px] font-semibold">Belum ada deposit</span>
                        <span>Yuk deposit sekarang dan mulai transaksi di Tokoisicash</span>
                     </div>
                  </div>
               <?php endif; ?>
            </section>
