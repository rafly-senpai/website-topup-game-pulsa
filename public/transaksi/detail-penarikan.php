<?php
   ...
   $withdrawal = getDataFromDatabase($withdrawalDb, "SELECT * FROM withdrawal WHERE withdrawal_id = '$withdrawalId'");
   if (empty($withdrawal)) {
      header("location:/public/tarik-saldo.php");
      exit;
   }
   ...
   <div class="max-w-3xl mx-auto min-h-screen bg-white overflow-hidden">
      ...
      <main class="p-5">
         <div class="flex flex-col gap-5">
            ...
            <!-- nav -->
            <div class="flex items-center justify-between gap-5">
               <button class="flex items-center gap-1 cursor-pointer" type="button" onclick="history.back();">
               ...
               <button class="text-gray-500 cursor-pointer" type="button" onclick="location.reload();">Reload</button>
            </div>
            
            <hr>
            
            <?php if ($withdrawal[0]["status"] === 0) : ?>
               <!-- pesan -->
               <div class="p-5 bg-gradient-to-b from-white to-gray-100 border shadow-md shadow-gray-200 rounded">
                  <div class="w-fit">
                     <span>Kami akan segera mengirim informasi selanjutnya ke nomor kamu melalui WhatsApp.</span>
                  </div>
               </div>
            <?php endif; ?>
               ...
               <div class="p-5 bg-white border-b">
                  <div class="flex items-center justify-center">
                     <div class="w-fit mx-auto text-center">
                        <span class="font-semibold">Detail Penarikan</span>
                     </div>
                  </div>
               </div>
               <div class="p-5">
                  <div class="flex flex-col">
                     <div class="py-3 bg-gray-50 border-b">
                        <div class="flex items-center justify-between gap-5">
                           ...
                           <span class="font-semibold">#<?= htmlspecialchars($withdrawal[0]["withdrawal_id"]); ?></span>
                        </div>
                     </div>
                     <div class="py-3 border-b">
                        <div class="flex items-center justify-between gap-5">
                           <span>Jumlah Penarikan</span>
                           <span class="font-semibold"><?= htmlspecialchars($withdrawal[0]["amount"]); ?></span>
                        </div>
                     </div>
                     <div class="py-3 bg-gray-50 border-b">
                        <div class="flex items-center justify-between gap-5">
                           <span>Total Diterima</span>
                           <span class="font-semibold">IDR <?= htmlspecialchars(number_format($withdrawal[0]["amount"] + 0, 0, ",", ".")); ?></span>
                        </div>
                     </div>
                     <div class="py-3 border-b">
                        <div class="flex items-center justify-between gap-5">
                           <span>Status</span>
                           <?php if ($withdrawal[0]["status"] === 0) : ?>
                              <span class="text-orange-500">Sedang diproses</span>
                           <?php elseif ($withdrawal[0]["status"] === 1) : ?>
                     ...
                     <div class="py-3 border-b">
                        <div class="flex items-center justify-between gap-5">
                           <span>Waktu Penarikan</span>
                           <span class="font-semibold"><?= htmlspecialchars($withdrawal[0]["time"]); ?></span>
                        </div>
                     </div>
                  </div>
               </div>
